...
export default async function DashboardPage() {
  ...
  const agentProfile = session.role === Role.AGENT ? await db.agentProfile.findUnique({ where: { userId: session.userId } }) : null;
  return <><div className="page-title"><div><h1>{heading}</h1><p>{session.role === Role.ADMIN ? "Monitor the network and resolve exceptions." : "Here’s what is moving today."}</p></div>{session.role !== Role.AGENT && <Link href="/dashboard/orders/new" className="button">+ New order</Link>}</div>{agentProfile && <AgentAvailability latitude={agentProfile.currentLatitude} longitude={agentProfile.currentLongitude} available={agentProfile.available} />}<section className="stats-grid"><div className="stat-card"><div className="stat-label">Total orders</div><div className="stat-value">{total}</div><div className="stat-note">All recorded deliveries</div></div><div className="stat-card"><div className="stat-label">Active</div><div className="stat-value">{active}</div><div className="stat-note">Currently in the network</div></div><div className="stat-card"><div className="stat-label">Delivered</div><div className="stat-value">{delivered}</div><div className="stat-note">Successfully completed</div></div><div className="stat-card"><div className="stat-label">Needs attention</div><div className="stat-value">{failed}</div><div className="stat-note">Failed delivery attempts</div></div></section><section className="card"><div className="card-head"><h2>Recent activity</h2><Link href="/dashboard/orders" className="button ghost small">View all →</Link></div>{recent.length ? <div className="table-wrap"><table className="table"><thead><tr><th>Tracking</th>{session.role === Role.ADMIN && <th>Customer</th>}<th>Route</th><th>Status</th><th>Charge</th><th>Updated</th></tr></thead><tbody>{recent.map((order) => <tr key={order.id}><td><Link className="tracking-code" href={`/dashboard/orders/${order.id}`}>{order.trackingNumber}</Link></td>{session.role === Role.ADMIN && <td>{order.customer.name}</td>}<td>{order.pickupZone.name} → {order.dropZone.name}</td><td><StatusPill status={order.status} /></td><td>₹{Number(order.totalCharge).toFixed(2)}</td><td>{order.updatedAt.toLocaleDateString("en-IN")}</td></tr>)}</tbody></table></div> : <div className="empty">No orders yet. Your first delivery will appear here.</div>}</section></>;
}
